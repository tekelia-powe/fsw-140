...
app.post('/insertcontacts', (req,res) => {
  ...
  let sql = "INSERT INTO contacts (firstName, lastName, age,saved, phone, email)  VALUES (?,?,?,?,?,?)";
  


  connection.query(sql,[firstName, lastName, age,saved, phone, email], (err, result) => {
if(err){
  throw err;
}
console.log("Data inserted successfully")
...
app.put('/editcontact/:contactId', (req,res) => {
  ...
  let sql = "UPDATE contacts SET firstname=?, lastName =?, age=?, saved = ?, phone =?, email =? WHERE id=?";

  connection.query(sql, [firstName, lastName, age,saved, phone, email, id], (err, result) => {
if(err){
  throw err;
}
res.send(result)
